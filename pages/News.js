...
export default function Media({ mediaData, metaData,  limit, pageCount }) {
  ...
  const fetchComments = async (currentPage) => {
    const res = await fetch(
      `${process.env.SERVER_LINK}/api/mediaCenter/?mediaCategoryID=1&flag=1&page=${currentPage}&limit=${limit}`
    );
                ...
                <Col lg={4}>
                  <div className="HomeMediaInfo">
                    ...
                    <div className="CeoLink">
                      <Link href={`/NewsDetails/${data.slug}/${data.id}`}>
                      {t("moreDetails")}
                      </Link>
                    </div>
                  </div>
...
export async function getServerSideProps({ query , locale }) {
  ...
  try {
    const [mediaRes, metaRes] = await Promise.all([
      axios.get(
        `${process.env.SERVER_LINK}/api/mediaCenter/?mediaCategoryID=1&flag=1&page=1&limit=${limit}`, {timeout: 15000 }
      ),
      axios.get(
        `${process.env.SERVER_LINK}/api/metaTags/lang?lang=${locale}&page=news`, {timeout: 15000 }
      ),
    ]);
