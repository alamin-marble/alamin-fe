...
export default function Media({ mediaData, metaData,  limit, pageCount }) {
  ...
  const fetchComments = async (currentPage) => {
    const res = await fetch(
      `${process.env.SERVER_LINK}/api/mediaCenter/?mediaCategoryID=2&flag=1&page=${currentPage}&limit=${limit}`
    );
...
export async function getServerSideProps({ query , locale }) {
  ...
  try {
    const [mediaRes, metaRes] = await Promise.all([
      axios.get(
        `${process.env.SERVER_LINK}/api/mediaCenter/?mediaCategoryID=2&flag=1&page=1&limit=${limit}`, {timeout: 15000 }
      ),
      axios.get(
        `${process.env.SERVER_LINK}/api/metaTags/lang?lang=${locale}&page=blog`, {timeout: 15000 }
      ),
    ]);
